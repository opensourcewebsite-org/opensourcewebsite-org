...
При личной переписке с пользователем бот удаляет лишние сообщения не касающиеся текущего запроса. При этом для вывода сообщения используется команда editMessageTextOrSendMessage. Если необходимо отправить несколько сообщений (например, мы выводим текствовое сообщение "Ваша геопозиция", после чего отправляем сообщение с геопозицией, а затем сообщение с клавиатурой для редактирования или возврата), то следует сначала вызвать команду editMessageTextOrSendMessage, а после нее sendLocation и sendMessage
