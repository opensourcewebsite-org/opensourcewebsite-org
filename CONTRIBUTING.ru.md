...
Для апгрейда (upgrade) данных в базе данных, создайте миграцию, название которой начинается на `upgrade_`. Для апгрейда данных в миграции используется метод `safeUp()`, запрещено использовать доступ к данным через модели, только через DAO (yii\db\Command).
Использование таких миграций необходимо для существующих баз данных, и для всех новых такие миграции будут удалены. В `down ()` и `safeDown ()` допускается только удаление объектов структуры базы данных (таблиц, полей, ключей, индексов).

Не используйте переменные такие как `$tableName` и `$tableOptions`.

Для указания Первичного ключа (Primary key), используйте `$this->primaryKey()->unsigned()`. Используйте `unsigned()` для столбцов, где это возможно.

Для добавления столбца с типом значений integer и диапазоном 0-255, используйте `$this->tinyInteger()->unsigned()`.

Для добавления столбца с типом значений integer и диапазоном 0-65535, используйте `$this->smallInteger()->unsigned()`.

Для добавления столбца с типом значений date (дата) в большинстве случаев используйте тип данных `$this->integer()->unsigned()`. Исключения - если столбец будет активно использоваться в запросах MySQL в качестве даты.

Обычно таблицы базы данных называются в единственном числе для перечисления любых объектов. Например, `user`, но не `users`.
