...
<br/>
<?= Yii::t('bot', 'Public Key') ?><?= !$stellar->isConfirmed() ? ' (' . Yii::t('bot', 'added {0}', Yii::$app->formatter->asRelativeTime($stellar->created_at)) . ')' : '' ?>: <?= ExternalLink::getStellarExpertAccountFullLink($stellar->getPublicKey()) ?><br/>
<br/>
<?php if (!$stellar->isConfirmed()) : ?>
<?= Emoji::WARNING ?> <?= Yii::t('bot', 'Confirm your Stellar account') ?>.<br/>
<?php if ($distributor) : ?>
<br/>
