<b><?= Yii::t('bot', 'Your referrals'); ?></b><br/>
<br/>
<?= Yii::t('bot', 'All new users, who have joined the Bot or Website through your referral link, become your referrals'); ?>.<br/>
<br/>
- <?= Yii::t('bot', 'You get rewards from your referrals for their purchases on the Website and websites of our partner companies, and from offline partners using a discount card'); ?>.<br/>
- <?= Yii::t('bot', 'Multi-level loyalty program, you get rewards from multiple referral levels, not only from first level'); ?>.<br/>
- <?= Yii::t('bot', 'User community decides what conditions will be in loyalty program'); ?>. <?= Yii::t('bot', 'You can participate in discuss process and vote for the conditions'); ?>.
